...
public class Tag08 {
    ...
    public static void part1() {
        ...
        for (int i = 0; i < input.length / 2; i++) {
            for (int j = 0; j < 4; j++) {
                //counting 8
                if (output[i][j].length() == 7) {
                    cnt++;
                }
                //counting 1
                if (output[i][j].length() == 2) {
                    for (int l = 0; l < 10; l++) {
                        if (cables[i][l].length() == 2) {
                            if (stringSort(cables[i][l]).equals(stringSort(output[i][j]))) {
                                cnt++;
                            }
                        }
                    }
                ...
                //counting 4
                if (output[i][j].length() == 4) {
                    for (int l = 0; l < 10; l++) {
                        if (cables[i][l].length() == 4) {
                            if (stringSort(cables[i][l]).equals(stringSort(output[i][j]))) {
                                cnt++;
                            }
                        }
                ...
                //counting 7
                if (output[i][j].length() == 3) {
                    for (int l = 0; l < 10; l++) {
                        if (cables[i][l].length() == 3) {
                            if (stringSort(cables[i][l]).equals(stringSort(output[i][j]))) {
                                cnt++;
                            }
                        }
    ...
    public static void part2() {
        ...
        for (int i = 0; i < input.length/2; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                for (int l = 0; l < 10; l++) {
                    if (output[i][j].equals(decoder(cables[i]).get(l))){
                        number[j] = Integer.toString(l);
                    }
                }
            }
            String s = buildString(number);
    ...
    private static HashMap<Integer, String> decoder(String[] numbers){
        HashMap<Integer, String> decodedNums = new HashMap<>();
        for (int i = 0; i < 10; i++) {
            //decoded 8
            if(numbers[i].length() == 7){
                decodedNums.put(8, numbers[i]);
            }
            //decoded 7
            if(numbers[i].length() == 3){
                decodedNums.put(7, numbers[i]);
            }
            //decoded 1
            if (numbers[i].length() == 2){
                decodedNums.put(1, numbers[i]);
            }
            //decoded 4
            if (numbers[i].length() == 4){
                decodedNums.put(4, numbers[i]);
            }
        }

        for (int i = 0; i < 10; i++) {
            if (numbers[i].length()==6){
                if (!(containsIn(decodedNums.get(1), numbers[i]))){
                    decodedNums.put(6, numbers[i]);
                }else{
                    if(containsIn(decodedNums.get(4), numbers[i])){
                        decodedNums.put(9, numbers[i]);
                    }
                    else {
                        decodedNums.put(0, numbers[i]);
                    }
                }
            }
        }
        String last = subtract(decodedNums.get(8), decodedNums.get(9));

        for (int i = 0; i < 10; i++) {
            if (numbers[i].length() == 5){
                if (containsIn(decodedNums.get(1), numbers[i])){
                    decodedNums.put(3, numbers[i]);
                }else {
                    if (containsIn(last, numbers[i])){
                        decodedNums.put(2, numbers[i]);
                    }else {
                        decodedNums.put(5, numbers[i]);
                    }
                }
            }
        }
    ...
    private static String subtract(String eight, String nine){
        String tmp = eight;
        for (int i = 0; i < 6; i++) {
            tmp = tmp.replace(Character.toString(nine.charAt(i)), "");
        }
        return tmp;
    }
